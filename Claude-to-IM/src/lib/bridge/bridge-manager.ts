...
import type { BridgeStore, CliSessionInfo } from './host.js';
...
/**
 * Format a relative time string from a timestamp (milliseconds).
 */
  ...
  switch (command) {
    ...
    case '/bind': {
      ...
      if (binding) {
        ...
        // Fallback: try to find a CLI session matching this ID
        const cliSession = findCliSession(store, args);
        if (cliSession) {
          const resumeResult = resumeCliSession(store, msg.address, cliSession);
    ...
    case '/list': {
      if (!store.listCliSessions) {
        response = 'CLI session scanning is not available.';
        break;
      }
      const sessions = store.listCliSessions({ limit: 20 });
      if (sessions.length === 0) {
        response = 'No local CLI sessions found.';
        break;
      }
      // Cache for /resume by number
      listCache.set(msg.address.chatId, { sessions, cachedAt: Date.now() });

      const lines = ['<b>本地 CLI 会话:</b>', ''];
      for (let i = 0; i < sessions.length; i++) {
        ...
        lines.push(`${i + 1}. ${icon} <code>${s.sdkSessionId.slice(0, 8)}</code>  ${escapeHtml(s.project)}`);
        lines.push(`   "${escapeHtml(prompt)}" (${timeAgo})`);
      }
      lines.push('');
    ...
    case '/resume': {
      ...
      // Try numeric index from /list cache
      const num = parseInt(args, 10);
      if (!isNaN(num) && num > 0 && String(num) === args.trim()) {
        const cached = getCachedList(msg.address.chatId);
        if (cached && num <= cached.length) {
          target = cached[num - 1];
        } else {
          // Re-scan if cache expired
          const freshSessions = store.listCliSessions({ limit: 20 });
          listCache.set(msg.address.chatId, { sessions: freshSessions, cachedAt: Date.now() });
      ...
      // Try UUID prefix or slug match
      if (!target) {
        target = findCliSession(store, args);
      }

      if (!target) {
        response = `未找到匹配 "${escapeHtml(args)}" 的会话。\n发送 /list 查看可用会话。`;
        break;
      }

      // Abort any running task on current session
      const oldBinding = router.resolve(msg.address);
      const st = getState();
      const oldTask = st.activeTasks.get(oldBinding.codepilotSessionId);
      if (oldTask) {
      ...
      response = resumeCliSession(store, msg.address, target);
      break;
    }
...
/**
 * Find a CLI session by UUID prefix or slug match.
 */
function findCliSession(store: BridgeStore, query: string): CliSessionInfo | null {
  if (!store.listCliSessions) return null;
  const sessions = store.listCliSessions({ limit: 50 });
...
/**
 * Resume a CLI session: create a new bridge session, upsert binding with
 * the CLI session's sdkSessionId. Returns the confirmation message.
 */
function resumeCliSession(store: BridgeStore, address: ChannelAddress, target: CliSessionInfo): string {
  const defaultModel = store.getSetting('bridge_default_model') || '';

  // Create a new bridge session with the CLI session's cwd
  ...
  // Upsert binding (creates/updates the binding record)
  ...
  // upsertChannelBinding ignores sdkSessionId in its input (hardcodes ''),
  // so we must explicitly patch it via updateChannelBinding.
  store.updateChannelBinding(binding.id, { sdkSessionId: target.sdkSessionId });
  ...
  return [
    `${icon} 已恢复 CLI 会话`,
    '',
    `Session: <code>${target.sdkSessionId.slice(0, 8)}...</code>`,
    `Project: <code>${escapeHtml(target.project)}</code>`,
    `CWD: <code>${escapeHtml(target.cwd)}</code>`,
    target.slug ? `Slug: <code>${escapeHtml(target.slug)}</code>` : '',
    `"${escapeHtml(prompt)}"`,
    '',
    '现在可以直接发消息继续对话。',
  ].filter(Boolean).join('\n');
